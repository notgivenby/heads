...
```text
initrd/init  (PID 1)
  └─ CONFIG_BOOTSCRIPT → initrd/bin/gui-init.sh     [board config]
       ...
       └─ calls initrd/bin/tpmr.sh                   [TPM abstraction]
            ├─ TPM1: calls `tpm` (tpmtotp util/tpm)  [CONFIG_TPM=y]
...
| `0x98e` | Hex code | TPM2 | `TPM2_RC_AUTH_FAIL` |
| `0x149` | Hex code | TPM2 | `TPM2_RC_NV_AUTHORIZATION` |
...
software presence.  `TPM_ResetLockValue` (in `util/resetlockvalue.c`)
